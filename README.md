...
**Menu photographs.** `js/menu-data.js` items accept an optional `img` path. Nine items carry one today — six mocktails and three munchies. Everything else still shows a tile tinted in the service colour carrying the category mark, a deliberate object rather than an empty frame, so the menu looks finished with only part of it photographed.

Six of the nine arrived as full marketing posters rather than product shots: portrait, with the drink name, the Picasso wordmark, and **Picasso's own counter price printed on them** — ₹119 for the mojito where this site lists ₹134. The difference is exactly the ₹15 per-item margin the listed prices already carry, so the poster was not wrong, it just showed the other number. Displaying it beside the site's price would read as overcharging, so each was cropped to the centre column: the glass survives, the price badge, wordmark and type do not. `crop.js` in the scratchpad did this through a Chromium canvas (no ImageMagick on the box) and also resized them, taking the set from 2.3 MB to about 520 KB.

Photos are deliberately **not** in the service worker's `PRECACHE` — the asset handler is stale-while-revalidate, so they cache on first view instead of making every install pay for half a megabyte.
...
**Tapping a photo expands it.** A tile is a 74px square, so it can only ever show a slice of a tall poster. Tapping one opens the whole image with the item's name, description, tag, this site's price and an Add button; it closes on the X, the backdrop or Escape. Only photographed items are tappable — the emoji tiles are not pretending to be. The large file is fetched on open, not with the page, so a menu of nine posters costs nothing until someone asks for one.

The expanded image comes from `img/menu/full/`, which is the poster with the printed price removed; `img/menu/` holds the small crop the tile uses. `crop.js` and `paint3.js` in the scratchpad produce them. Painting the price out took three attempts worth recording, because each naive fill left something behind: cloning the band straight above the badge dragged down a sliver of the glass (it tapers, so it is wider higher up), and offsetting that sample sideways pulled in the poster's own gold border. The fill is now tiled from one window — x 754–1004 in the band above the badge — which is the only region simultaneously clear of the glass, the badge and the border. The badge's y position drifts up to 140px between posters, so those rects are per file; a shared one clipped the ice cubes under Bubble Berrygum.
...
## Weekly vegetable planner (`weekly.html`)

A once-a-week flow for fresh vegetables: the customer sees the seven days of the coming week, puts what they want under each day — cauliflower Monday, tomatoes Wednesday — picks **one** delivery time slot covering the whole week, and submits it as a single plan. That plan becomes one ordinary order per day they filled in, and the owner routes each day separately.

Two decisions shape the whole design:

- **The delivery slot is capped, not exclusive.** Slots are a real customer promise ("between 8 and 10 am"), and each one holds a fixed number of customers (`config.js`'s `WEEKLY_SLOT_CAPACITY`, overridable per week). Once a slot is full it greys out on the planner and the server refuses it. That keeps a morning's load matched to how many drivers are actually working, without the absurdity of one person per slot.
- **Order within a day is decided by geography, not signup time.** The problem at 10-20 customers isn't slot collisions, it's a driver zigzagging across town because people were assigned in the order they happened to sign up. Each day's stops are sequenced from the dark store outward and split across drivers, capped per driver.

**The cycle, run by hand from `admin.html`'s 🗓️ Weekly tab:**
1. **Open the planner** — a cutoff (date + time), the date the delivery week starts, and how many customers each slot holds.
2. **Customers plan their week** on `weekly.html`: vegetable chips plus free text under each day, one slot for the week. The plan lives in its own cart (`warpx_weekly_cart` via `js/cart.js`'s `setCartKey()`) so it never mixes with a same-day grocery cart, and the day is carried in each line's `note`. Checkout is the *same* `checkout.html` — address book, COD/UPI — with items grouped under dated headings and the timing card replaced by the chosen slot.
3. **Close the planner** — by hand, or automatically once the cutoff passes (`lib/weekly.js`'s `isWindowOpen()` checks both).
4. **Route one day at a time** — pick a day from the day strip, choose which approved drivers are working and a max-per-driver cap, preview the stop order and rough distance, then commit. Monday being underway never blocks routing Friday.
5. **Drivers see their stops grouped by day** in the Driver Hub — today expanded, later days collapsed to a line — each day with its own combined Google Maps link, worked with the same "Picked up"/delivery-code buttons as any other order.

**A plan is created atomically (`POST /api/weekly/plans`).** A week is several orders, so it can't go through `POST /api/orders`, which makes exactly one: a failure partway would leave someone holding half a week, and the slot check would race against itself across the requests. One endpoint writes every day's order in a single transaction. Resubmitting before the cutoff *replaces* the week outright (new orders, new codes), and `weekly_plans`' `UNIQUE(window_id, user_id)` means a customer can only ever hold one plan per week.

**Pricing is one flat weekly delivery fee**, set by `WEEKLY_FEE_MODE`/`WEEKLY_FLAT_FEE` in `lib/fee.js` — the only two constants that decide it, read by both the checkout quote and the server charge so they can't drift. Per-day billing is a one-word change (`"per-day"`), but it's the wrong default here: vegetables are priced at the market, so every day's basket sits on the ₹0 slab — the dearest one — and a five-day plan would bill ₹200 of delivery on goods the site can't even price yet. The flat fee is **split across the days** rather than parked on one order, because driver bonuses (`lib/tier.js`) and the owner's earnings panel both read `delivery_fee` per order; the shares always sum to exactly the quoted total.

**Kept separate from the ASAP/scheduled system on purpose:** a plan's orders never touch `validateSchedule`/`isOpenNow` — they land at the slot the customer already chose, not a time picked at checkout. They're also invisible to the driver job board (`GET /api/orders/available` excludes anything with a `weekly_window_id`) for their entire life before routing, so a driver can only get one through the owner's assignment step, never by self-claiming. That's what makes the routing mean anything.

**Known limitations:**
- **Vegetables aren't priced up front.** Items are quoted at the market and confirmed on the day, so a plan shows a delivery total and "vegetables priced at the market" rather than a basket price.
- **One-time database rebuild.** A database created before this redesign has the old single-delivery-date `weekly_windows`; `db.js` drops that table once on startup and detaches its orders (they survive as ordinary orders). Only test windows existed, and `warpx.db` is gitignored, so nothing real is lost.
- **Reassignment locks per day once underway.** A day's routes can be re-run to fix a wrong driver pick right up until one of its orders moves past `placed` — after that that day is final, since pulling a stop from a driver already out would leave someone waiting for nobody. Other days stay routable.
- **No automatic repeat.** Each week is planned fresh; regulars get a "copy last week's list" button rather than a standing subscription.
