...
/* The environment wins over the committed value, so a one-off
   `GOOGLE_CLIENT_ID=… npm start` can point at a second Google project or a
   different domain without editing anything.

   Set-but-empty is treated as a deliberate "off" rather than falling through
   to config.js — otherwise `GOOGLE_CLIENT_ID= npm start` would silently keep
   Google sign-in on, and there'd be no way to switch it off for one run
   without editing a committed file. */
const CLIENT_ID = process.env.GOOGLE_CLIENT_ID !== undefined
